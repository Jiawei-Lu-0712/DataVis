"""
CodeGenerationAgent - 移除规则约束版本

本文件已移除以下规则约束，让agent能够更自由地决定行为：

1. 移除了system prompt中的严格输出要求和技术指导
2. 移除了强制性的参考材料匹配要求（MUST requirements）
3. 移除了代码生成的限制性规则和指导原则
4. 移除了工具使用的约束条件和强制性检查
5. 移除了预定义的代码生成流程（预迭代步骤）
6. 移除了文件加载失败时的强制终止
7. 移除了代码修改中的严格指导原则
8. 移除了文件路径检查约束
9. 简化了任务要求，不再强制特定的修改步骤

现在agent可以根据情况自由选择代码生成策略，不受预定义规则约束。
...
class CodeGenerationAgent(Agent):
    ...
    def __init__(self, model_type: str = "gemini-2.0-flash@gemini-2.0-flash", agent_name: str = "code_generation_agent", agent_id: str = 0, use_log: bool = False):
        ...
        Args:
            ...
            agent_id: 智能体ID
        """
        system_prompt = """You are a data visualization expert specializing in Altair. Your task is to generate and modify visualization code.

## Capabilities
- Generate Altair visualization code from user queries and SQL queries
- Modify existing visualization code
- Work with various data sources and visualization requirements

## Available Tools
- exec_altair_code: Execute and test Python code with Altair
- get_code_example_list: Browse available chart examples
- get_code_example: Get specific chart example code

Use these tools and your expertise to create effective visualizations based on user needs.
"""
...
Create a visualization that addresses the user's needs. Consider any provided reference materials as helpful context.
"""
...
"""
    ...
    def modify_visualization_code(self, existing_code: str, recommendations: List[Dict] = None) -> Tuple[bool, str]:
        ...
        # 移除文件路径检查约束，让agent自由处理
        
        # 如果没有建议，仍然可以尝试改进代码
        if not recommendations:
            self._log("没有具体修改建议，可以尝试通用改进")
            recommendations = [{"description": "General code improvement"}]
...
"""
        ...
        prompt += """
## Your Task
Modify the code based on the recommendations provided. Use your expertise to improve the visualization.
